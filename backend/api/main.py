...
from backend.ai.integration import generate_ai_response
from backend.reports.pdf_gen import generate_proposal_pdf

# Ensure SQLite tables exist
Base.metadata.create_all(bind=engine)

app = FastAPI(title="AI Project Estimator & Proposal Generator API", version="1.0.0")
...
def create_project_estimate(payload: EstimateRequest, db: Session = Depends(get_db)):
    ...
    calls AI model (or fallback templates) for summary/stack/risks, saves to SQLite,
    generates a proposal PDF, and returns the full structured response.
    """
    try:
